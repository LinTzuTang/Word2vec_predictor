...
import numpy as np
import pandas as pd
from word2vec_encoding import word2vec_encoding
import argparse
...
def main(input_fasta_name,output_csv_name):
    ...
    dat = word2vec_encoding(input_fasta_name)
    data = np.array(list(dat.values())) 
    # load model
    root=os.path.dirname(__file__)
    model = load_model(root+'/../model/word2vec.h5')
    # predict
...
if __name__ == '__main__':   
    parser = argparse.ArgumentParser(description='word2vec predictor')
